...
import {
  getUploadUrl,
  r2PublicBaseUrlIssue,
  r2PublicBaseUrlOrNull,
  sanitizeFilename,
} from "./r2";
...
describe("r2PublicBaseUrlIssue", () => {
  ...
  it("returns null when everything is configured", () => {
    ...
    process.env.R2_PUBLIC_BASE_URL = "https://pub-abc.r2.dev";
    expect(r2PublicBaseUrlIssue()).toBeNull();
  });

  it("reports missing R2 credentials", () => {
    delete process.env.R2_ACCOUNT_ID;
    delete process.env.R2_ACCESS_KEY_ID;
    delete process.env.R2_SECRET_ACCESS_KEY;
    delete process.env.R2_BUCKET;
    delete process.env.R2_PUBLIC_BASE_URL;
    expect(r2PublicBaseUrlIssue()).toContain("R2_ACCOUNT_ID");
  });

  it("reports a missing or invalid R2_PUBLIC_BASE_URL", () => {
    process.env.R2_ACCOUNT_ID = "acct";
    process.env.R2_ACCESS_KEY_ID = "key";
    process.env.R2_SECRET_ACCESS_KEY = "secret";
    process.env.R2_BUCKET = "bucket";
    delete process.env.R2_PUBLIC_BASE_URL;
    expect(r2PublicBaseUrlIssue()).toContain("R2_PUBLIC_BASE_URL");
  });

  it("reports when R2_PUBLIC_BASE_URL points at the S3 endpoint", () => {
    process.env.R2_ACCOUNT_ID = "acct";
    process.env.R2_ACCESS_KEY_ID = "key";
    process.env.R2_SECRET_ACCESS_KEY = "secret";
    process.env.R2_BUCKET = "bucket";
